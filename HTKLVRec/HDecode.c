...
static FSLM *lm_htk;                /* language model */
ModelT *lm_kenlm;                /* language model using kenlm*/
...
//Model* Create_kenlm(const char* file){
...
//	      switch(model_type) {
//		      case PROBING:
//			      lm_kenlm = Create_kenlm<lm::ngram::ProbingModel>(langfn);
//			      break;
//		      case REST_PROBING:
//			      lm_kenlm = Create_kenlm<lm::ngram::RestProbingModel>(langfn);
//			      break;
//		      case TRIE:
//			      lm_kenlm = Create_kenlm<TrieModel>(langfn);
//			      break;
//		      case QUANT_TRIE:
//			      lm_kenlm = Create_kenlm<QuantTrieModel>(langfn);
//			      break;
//		      case ARRAY_TRIE:
//			      lm_kenlm = Create_kenlm<ArrayTrieModel>(langfn);
//			      break;
//		      case QUANT_ARRAY_TRIE:
//			      lm_kenlm = Create_kenlm<QuantArrayTrieModel>(langfn);
//			      break;
//		      default:
//			      HError (9999, "LoadKenLM:Unrecognized kenlm model type %d\n", model_type);
//	      }
//      }else{
//	      lm_kenlm = Create_kenlm<ProbingModel>(langfn);
//      }
   }
   else {
      net = NULL;
   ...
   dec = CreateDecoderInst (&hset, lm_kenlm, nTok, TRUE, useHModel, outpBlocksize,
                            (Boolean)(bestAlignMLF ? TRUE : FALSE),
                            modAlign);
...
void DoRecognition (DecoderInst *dec, char *fn)
{
   ...
   if (latRescore) {
      ...
      lm_htk = CreateLMfromLat (&lmHeap, latfn, lat, &vocab);
      dec->lm = lm_htk;
   }
