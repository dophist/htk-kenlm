...
{
	return IN10*lm_kenlm->Score(src, word_index, dest);
	//return IN10*lm_kenlm->Score(src, vocab_kenlm->Index(pWord), dest);
}
...
StateT LMInitial_kenlm(ModelT *lm_kenlm)
{
	//lm::ngram::State state;
	return lm_kenlm->BeginSentenceState();
	//return state;
}
...
//   		char* pWord = dec->net->pronlist[i]->word->wordName->name;
//		prob = lm_kenlm->Score(src,vocab_kenlm->Index(pWord),dest);
//		if( prob > maxProb )
//			maxProb = prob;
//   	}
//   maxProb = -maxProb*log(10);
//   return maxProb;
//}
...
/* Fast_LMLA_LMState
     ...
     and use 2gram state
*/
StateT Fast_LMLA_LMState_kenlm (ModelT *lm_kenlm, StateT src)
{
