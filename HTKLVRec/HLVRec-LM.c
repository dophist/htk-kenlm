...
/* LMCacheTransProb
     ...
     return the (scaled!) LM transition prob and dest LMState for the given LMState and PronId using kenlm
*/

static LMTokScore LMCacheTransProb_kenlm (DecoderInst *dec, ModelT *lm_kenlm, /*const VocabularyT *vocab_kenlm,*/
                                    StateT &src, PronId pronid, StateT &dest)
{

	//char* pWord = dec->net->pronlist[pronid]->word->wordName->name;
	unsigned int word_index = dec->net->pron2wordIndex[pronid];
	//float prob = LMTransProb_kenlm (lm_kenlm, vocab_kenlm, src, pWord, dest);
	float prob = LMTransProb_kenlm (lm_kenlm, /*vocab_kenlm,*/ src, word_index, dest);

	 if( pronid == 1 )
		 return 0.0;

	 //printf("%s %f %f\n",pWord,dec->lmScale * LMTransProb_kenlm (lm_kenlm, vocab_kenlm, src, pWord, dest),dec->lmScale*prob);

	 //return dec->lmScale * LMTransProb_kenlm (lm_kenlm, vocab_kenlm, src, pWord, dest);
	 return dec->lmScale * prob;
}
...
*/
static LMTokScore LMCacheLookaheadProb_kenlm (DecoderInst *dec, StateT &lmState, int lmlaIdx, Boolean fastlmla)
{
   LMCache *cache;
   LMTokScore lmscore;
   ...
   if (fastlmla) {      /* #### should only go to fast LMState if real one is not cahced */
      lmState = Fast_LMLA_LMState_kenlm (dec->lm_kenlm, lmState);
   }
   ...
   /* now entry points to the place to store the prob we are about to calulate */
   {
	 ...
	 //}
         ...
         lmscore = LZERO;
         for (i = 0; i < laNode->n; ++i) {
            score = LMCacheLookaheadProb_kenlm (dec, lmState, laNode->lmlaIdx[i], FALSE);
            if (score > lmscore)
               lmscore = score;
         }
      }
                                        ...
                                        int lmlaIdx, Boolean fastlmla)
{
   ...
   /* now entry points to the place to store the prob we are about to calulate */
   {
      ...
      if (lmlaIdx < laTree->nNodes) {        /* simple node */
         ...
         laNode = &laTree->node[lmlaIdx];
         ++cache->laMiss;
         lmscore = dec->lmScale * LMLookAhead (dec->lm, lmState, laNode->loWE, laNode->hiWE);
      }
      else {         /* complex node */
         CompLMlaNode *laNode;
         LMTokScore score;
