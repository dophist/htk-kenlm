   ...
   if (hmm->tIdx < 0) {
      ...
      for (i = N-2; i >=2; --i) {             /* for all internal states, except N-1 */
         ...
         if (ts->n > 0) {   /*  && (trP[i][i+1] > LSMALL)) { */
            ...
            MergeTokSet (dec, ts, ts+1 , trP[i][i+1], (Boolean)(!mergeTokOnly));
      ...
      if ((instTS[0].n > 0) && (trP[1][2] > LSMALL)) {
         MergeTokSet (dec, &instTS[0], &instTS[1], trP[1][2], (Boolean)(!mergeTokOnly));
      }
      ...
      for (j = 2; j < N; ++j) {
         ...
         for (i = 1; i < N; ++i) {
            ...
            if ((instTS[i-1].n > 0) && (trP[i][j] > LSMALL)) {
               MergeTokSet (dec, &instTS[i-1], &tempTS[j-1], trP[i][j], (Boolean)(!mergeTokOnly));
            }
         }
         if (tempTS[j-1].n > 0) {
...
static void HandleWordend (DecoderInst *dec, LexNode *ln)
{
   ...
   */
   ...
   for (i = 0; i < ts->n; ++i) {
      ...
      //find wordstr for wordidx pronid
	  lmScore = LMCacheTransProb_kenlm (dec, dec->lm_kenlm, /*dec->vocab_kenlm,*/ tok->lmState_kenlm, pronid, dest_kenlm);
