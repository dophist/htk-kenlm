...
void InitLVRec(void);
DecoderInst *CreateDecoderInst(HMMSet *hset, ModelT *lm_kenlm, int nTok, Boolean latgen, 
                               Boolean useHModel,
                               ...
                               Boolean modAlign)
{
   ...
   //dec->lm = lm;
   dec->lm_kenlm = lm_kenlm;
   dec->vocab_kenlm = lm_kenlm->GetVocabulary_ptr();
   dec->hset = hset;
...
{       
   ...
   */
   ...
   dec->net->start->inst->ts[0].relTok[0].lmState_kenlm = LMInitial_kenlm (dec->lm_kenlm);
...
/*    inst->ts = NewTokSetArray (dec, N);          /\*  size is  N * sizeof (TokenSet) *\/ */
   ...
   inst->ts = NewTokSetArrayVar (dec, N, (Boolean)(l == LAYER_SIL));          /*  size is  N * sizeof (TokenSet) */
#ifdef DEBUG_TRACE
   if (trace & T_ACTIV)
