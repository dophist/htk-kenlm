...
Boolean HardAssign(AdaptXForm *xform)
{
   ...
   return (Boolean)((akind == TREE) || (akind == BASE));
}

static Boolean StoreObsCache(AdaptXForm *xform)
{
   XFormKind xkind = xform->xformSet->xkind;
   return (Boolean)((xkind == CMLLR)  || (xkind == MLLRCOV) || (xkind == SEMIT));
}

static Boolean StoreAdaptMean(AdaptXForm *xform)
{
   XFormKind xkind = xform->xformSet->xkind;
   return (Boolean)((xkind == MLLRMEAN) || (xkind == MLLRCOV));
}

static Boolean StoreAdaptCov(AdaptXForm *xform)
{
   XFormKind xkind = xform->xformSet->xkind;
   return (Boolean)(xkind == MLLRVAR);
}
...
static int GetVecSizeClass(BaseClass *bclass, int classIdx)
{
   ILink i;
   MixtureElem *me;

   /* currently does not check consistency of vector sizes */
   i=bclass->ilist[classIdx];
   me = (MixtureElem *)i->item;
   return VectorSize(me->mpdf->mean);
}

static IntVec GetBlockSize(AdaptXForm *xform, int classIdx)
{
   ...
   if (strmProj) {
      ...
      blockSize = CreateIntVec(xform->mem,1);
      blockSize[1] = GetVecSizeClass(xform->bclass,classIdx);
   }
   return blockSize;  
}
...
static void CreateBaseTriMat(MemHeap *x, MixPDF *mp, AdaptXForm *xform, int classIdx)
{
  TriMat *tm;
  int vsize = VectorSize(mp->mean);
  IntVec blockSize = GetBlockSize(xform,classIdx);
  RegAcc *regAcc, *ra;
  MixPDF *me;
  BaseClass *bclass;
  ILink i;
  ...
  bclass = xform->bclass;
  for (i=bclass->ilist[classIdx]; i!=NULL; i=i->next) { 
    if (xform->info->accBTriMat) {
...
void GenXForm(RegNode *node, AdaptXForm *xform, IntVec classes)
{
   ...
   while (classes[classIdx] == 0) classIdx++;
   if (strmProj)
      accs = CreateAccStruct(&gstack,xform,xform->hset->vecSize,GetBlockSize(xform,classIdx));
   else 
      accs = CreateAccStruct(&gstack,xform,node->vsize,GetBlockSize(xform,classIdx));
