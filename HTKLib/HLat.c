...
{
   startWord = GetLabId (start ? start : (char*)"!SENT_START", TRUE);
   startLMWord = GetLabId (startLM ? startLM : (char*)"<s>", TRUE);
   endWord = GetLabId (end ? end : (char*)"!SENT_END", TRUE);
   endLMWord = GetLabId (endLM ? endLM : (char*)"</s>", TRUE);
   nullWord = GetLabId ((char*)"!NULL", TRUE);
}
