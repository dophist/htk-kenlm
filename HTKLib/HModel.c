...
static ReturnStatus GetOption(HMMSet *hset, Source *src, Token *tok, int *nState)
{
   ...
   case NUMSTATES:
      ...
      *nState=nSt;
      break;
   case PARMKIND:    
      OWarn(hset,(Boolean)(hset->pkind==tok->pkind),"parmKind");
   ...
   case GENDUR:
      ...
      OWarn(hset,(Boolean)(hset->dkind==dk),"durKind");
      hset->dkind = dk; 
      break;
   case HMMSETID:
   ...
   case VECSIZE:
      ...
      OWarn(hset,(Boolean)(hset->vecSize==vs),"vecSize");
      hset->vecSize = vs;
      break;
   case PROJSIZE:
      if (!ReadShort(src,&vs,1,tok->binForm)){
      ...
      OWarn(hset,(Boolean)(hset->projSize==vs),"projSize");
      hset->projSize = vs;
      break;
   case STREAMINFO:
      ...
      OWarn(hset,(Boolean)(hset->swidth[0]==sw[0]),"swidth[0]");       
      for (i=0; i<=sw[0]; i++) hset->swidth[i] = sw[i];
      break;
   case DIAGCOV:
      OWarn(hset,(Boolean)(hset->ckind==DIAGC),"covKind");
      hset->ckind = DIAGC; 
      break;
   case FULLCOV:
      OWarn(hset,(Boolean)(hset->ckind==FULLC),"covKind");
      hset->ckind = FULLC; 
      break;
   case XFORMCOV:
      OWarn(hset,(Boolean)(hset->ckind==XFORMC),"covKind");
      hset->ckind = XFORMC; 
      break;
   case INVDIAGCOV:
      OWarn(hset,(Boolean)(hset->ckind==INVDIAGC),"covKind");
      hset->ckind = INVDIAGC; 
      break;
   case LLTCOV:
      OWarn(hset,(Boolean)(hset->ckind==LLTC),"covKind");
      hset->ckind = LLTC; 
      break;
   default: 
                             ...
                             int M, int s, Vector tpdf)
{
   ...
   isNew = (Boolean)( hset->tmRecs[s].mixId == NULL );
   if (isNew) {
...
static BaseClass* GetBaseClass(HMMSet *hset,Source *src, Token *tok)
{
  ...
  if (tok->sym == MMFIDMASK) { 
	...
	HError(999,"Error reading classes in BaseClass");
      ...
      PItemList(&ilist,&type,hset,src,(Boolean)(trace&T_PAR));
...
void PrintHSetProfile(FILE *f, HMMSet *hset)
{
   ...
   for ( ck = 0; ck < NUMCKIND; ++ck ) 
	 ...
	 fprintf (f, "   CK %-8s = %d mixcomps\n", CovKind2Str ((CovKind)ck, buf), hset->ckUsage[ck]);
...
void SetCovKindUsage (HMMSet *hset)
{
   ...
   if (hset->ckind == NULLC) {
      ...
      for (ck = (CovKind)0; ck < (CovKind)NUMCKIND; ++ck)
         if (hset->ckUsage[ck] > 0) {
            ++nCK;
            lastSeenCK = (CovKind)ck;
         }
      if (nCK == 1)
...
void SaveAllXForms(HMMSet *hset, char *fname, Boolean binary)
{
  ...
  binary = (Boolean)(binary||saveBinary);
