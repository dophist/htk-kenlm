...
ReturnStatus WriteOneLattice(Lattice *lat,FILE *file,LatFormat format)
{
   ...
   if ((format&HLAT_TIMES) || !(format&HLAT_ALABS)) {
      for (i=0;i<lat->nn;i++) {
         ...
         rorder[order[i]]=i;
         ln->n = i;
         OutputIntField('I',i,(Boolean)(format&HLAT_LBIN),"%-4d",file);
         if (format&HLAT_TIMES)
            OutputFloatField('t',ln->time / lat->tscale,(Boolean)(format&HLAT_LBIN),"%-5.2f",file);
         if (!(format&HLAT_ALABS)) {
            if (ln->word==lat->voc->subLatWord && ln->sublat!=NULL)
               ...
               if ((format&HLAT_PRON) && ln->v>=0)
                  OutputIntField('v',ln->v,(Boolean)(format&HLAT_LBIN),"%-2d",file);
   ...
   for (i=0;i<lat->na;i++) {
      ...
      OutputIntField('J',i,(Boolean)(format&HLAT_LBIN),"%-5d",file);
      st=rorder[la->start-lat->lnodes];
      en=rorder[la->end-lat->lnodes];
      OutputIntField('S',st,(Boolean)(format&HLAT_LBIN),"%-4d",file);
      OutputIntField('E',en,(Boolean)(format&HLAT_LBIN),"%-4d",file);
      if (format&HLAT_ALABS) {
         if (la->end->word!=NULL) 
            fprintf(file,"W=%-19s ",
            ...
            fprintf(file,"W=%-19s ","!NULL");
         if ((format&HLAT_PRON) && ln->v>=0)
            OutputIntField('v',la->end->v,(Boolean)(format&HLAT_LBIN),"%-2d",file);
      }
      if (!(lat->format&HLAT_SHARC) && (format&HLAT_ACLIKE))
         OutputFloatField ('a', ConvLogLikeToBase(lat->logbase, la->aclike), (Boolean)(format&HLAT_LBIN), "%-9.2f", file);
      if (format&HLAT_LMLIKE) {
         if (lat->net==NULL)
            OutputFloatField('l', ConvLogLikeToBase(lat->logbase, la->lmlike*lat->lmscale+lat->wdpenalty),
                             (Boolean)(format&HLAT_LBIN),"%-8.2f",file);
         else
            OutputFloatField('l',ConvLogLikeToBase(lat->logbase, la->lmlike), (Boolean)(format&HLAT_LBIN), "%-7.3f", file);
      }
      if (!(lat->format&HLAT_SHARC) && (format&HLAT_PRLIKE))
         OutputFloatField('r', ConvLogLikeToBase(lat->logbase, la->prlike), (Boolean)(format&HLAT_LBIN), "%-6.2f", file);
