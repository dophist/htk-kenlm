...
{
   return (Boolean)(FindConfParm(list,size,name,AnyCKind) != -1);
}
...
static Boolean IsVAXOrder(void)
{
   ...
   *pc = 1; *(pc+1) = 0;         /* store bytes 1 0 */
   return (Boolean)(x==1);          /* does it read back as 1? */
}
...
{
   return (RawReadShort(src,s,n,binary,(Boolean)(vaxOrder && !natReadOrder)));
}

/* EXPORT->ReadInt: read n ints from src in ascii or binary */
Boolean ReadInt(Source *src, int *i, int n, Boolean binary)
{
   return (RawReadInt(src,i,n,binary,(Boolean)(vaxOrder && !natReadOrder)));
}

/* EXPORT->ReadFloat: read n floats from src in ascii or binary */
Boolean ReadFloat(Source *src, float *x, int n, Boolean binary)
{
   return (RawReadFloat(src,x,n,binary,(Boolean)(vaxOrder && !natReadOrder)));
}
...
Boolean RMatch(char *s,char *p,int slen,int minplen,int numstars)
{
   ...
   if (minplen>slen)
      return FALSE;
   if (*p == '*')
      return (Boolean)(RMatch(s+1,p+1,slen-1,minplen,numstars-1) || RMatch(s,p+1,slen,minplen,numstars-1) || RMatch(s+1,p,slen-1,minplen,numstars));
   if (*p == *s || *p == '?')
			...
			int slen,int minplen,int numstars)
{
   ...
   if (slen==0 && minplen==0)
      ...
      match= (Boolean)((SpRMatch(s+1,p,spkr,slen-1,minplen,numstars) || SpRMatch(s,p+1,spkr,slen,minplen,numstars-1) || SpRMatch(s+1,p+1,spkr,slen-1,minplen,numstars-1)));
   }
   else if (*p == '%') {
      *spkr=*s,spkr[1]=0;
