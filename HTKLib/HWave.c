...
static Boolean natReadOrder = (Boolean)FALSE;    /* Preserve natural read byte order*/
...
void InitWave(void)
{
   ...
   numParm = GetConfig("HWAVE", (Boolean)TRUE, cParm, MAXGLOBS);
   if (numParm>0){
...
static Boolean MustSwap(SrcOrder so)
{
   ...
   if (GetConfStr(cParm,numParm,"BYTEORDER",bos)) { /* force required order */
      return (Boolean)( (strcmp(bos,"VAX") == 0) ? !vaxOrder : vaxOrder );
   } else
      switch(so) {
      case VAXSO:     return (Boolean)(!vaxOrder);
      case SUNSO:     return (Boolean)vaxOrder;
      case UNKNOWNSO: return (Boolean)FALSE;
      }
...
static long GetNISTHeaderInfo(FILE *f, Wave w, InputAction *ia)
{
   ...
   Boolean interleaved = (Boolean)FALSE;
   long nS,sR,sS, cC;
   long dataBytes;
...
static int GetShortPackBlock(char **inData, short **outData)
{
   ...
   for (i=0; i<nSamp; i++,out++) {
      *out = 0;
      ...
      negative = (Boolean)( buf & bitValue[7-charBits] );
...
static long GetAIFFHeaderInfo(FILE *f, Wave w, InputAction *ia)
{
   ...
   Boolean hasCC=(Boolean)FALSE, hasSC=(Boolean)FALSE;
   ...
   *ia = (InputAction) 0;
   ...
   while (!(hasCC && hasSC)) {
      ...
      if (ch1.id == ccid){  /* common chunk found */
         hasCC=(Boolean)TRUE; commchunk1=ch1;commchunk2=ch2;
      }
      if (ch1.id == scid){  /* sound chunk found */
         hasSC=(Boolean)TRUE; sndStart=fPtr;
      }
      fPtr += ch1.size + 8;
   }
                    ...
                    short kind, Boolean *bSwap)
{
   ...
   if (!natWriteOrder && vaxOrder){
      ...
      SwapShort(&hdr.sampSize);
      SwapShort(&hdr.sampKind);
      if (bSwap!=NULL) *bSwap=(Boolean)TRUE;
   }
   else
      if (bSwap!=NULL) *bSwap=(Boolean)FALSE;
